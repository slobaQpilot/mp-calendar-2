...
class mpCalendar extends GestureEventListeners(PolymerElement) {
  static get template() {
    return html`
      ...
      <iron-a11y-keys
        id="a11y"
        target="{{target}}"
        keys="up down left right tab space"
        on-keys-pressed="{{chosen}}"
      ></iron-a11y-keys>
        ...
        <div id="header">
          <div class="month-display">
            ...
            <div id="currentMonth">
              ...
              <span
                class="todayDate"
                on-click="goToCurrentDate"
                title="Click to go on the today date"
              >
                <div class="show-inner-date">{{calendarDay}}</div>
                <svg
                  class="calendar-icon-todayDay"
  ...
  connectedCallback() {
    ...
    var d = moment.utc();
    var minYear = d.year() - this.minYear;
    var maxYear = d.year() + this.maxYear;
  ...
  _addListeners() {
    this.$.mpCalendar.addEventListener(
      "click",
      this._selectionHandler.bind(this)
    );
  }

  _removeListeners() {
    this.$.mpCalendar.removeEventListener(
      "click",
      this._selectionHandler.bind(this)
    );
  }
  ...
  _checkChosen() {
    if (this.chosen !== "" && this.chosen !== undefined) {
      var date = moment.utc(this.chosen);
      this.showDate.year = date.year();
  ...
  _selectionHandler(e) {
    if (e.target.getAttribute("data-date") == null) return;

    var dataDate = e.target.getAttribute("data-date");
    var dateObj = moment.utc(dataDate);

    this.chosen = dataDate;
    this.showDate.year = dateObj.year();
    this.date = {
      year: this.showDate.year,
  ...
  _chosenHandler(e) {
    var chosenDate = moment.utc(e);
    var chosenMonth = chosenDate.month();

    if (chosenMonth == this.date.date.month()) {
      ...
      if (e == "" || e == null) {
        ...
        this.date.day = chosenDate.toDate();
      }
    } else {
      this.showDate = {
        month: chosenMonth,
  ...
  prevMonthHandler() {
    this.set(
      "showDate.month",
      this.showDate.month <= 0 ? 11 : this.showDate.month - 1
    );
    this.set(
      "showDate.year",
      this.showDate.month == 11 ? this.showDate.year - 1 : this.showDate.year
    );
    this.chosen = "";
  ...
  nextMonthHandler() {
    this.set(
      "showDate.month",
      this.showDate.month == 11 ? 0 : this.showDate.month + 1
    );
    this.set(
      "showDate.year",
      this.showDate.month <= 0 ? this.showDate.year + 1 : this.showDate.year
    );
    this.chosen = "";
  ...
  _dateChanged(newDate, oldDate) {
    ...
    var year = parseInt(newDate.year) || date.year();
    ...
    this.currentMonth = this.monthLabels[this.showDate.month];
    this.calendarDay = this.date.day != null ? day : date.toDate();

    if (!!newDate && !!oldDate) {
      if (newDate.date.month() > oldDate.date.month()) {
        this._initCalandar(this.showDate.month, this.showDate.year);
        this._fire("nextMonth", this.showDate);
      }
      if (newDate.date.month() < oldDate.date.month()) {
  ...
  _initCalandar(month, year) {
    var today = moment.utc();
    var thisDay = today.toDate();
    var thisMonth = today.month() + 1;
    var dayOfMonthStart =
      moment.utc([year, month, 1]).day() == 0
        ? 7
        : moment.utc([year, month, 1]).day(); // which day the month starts (0 - 6)
    ...
    for (var i = 0; i < showDaysInMonth; i++) {
      ...
      if (i < dayOfMonthStart - this.firstDayOfWeek) {
        ...
        if (this.disablePrevDays) {
          ...
          var number = this._dayNumber(
            previousMonthDays - dayOfMonthStart + (i + 1 + this.firstDayOfWeek),
            previousMonth + 1,
            previousYear,
            day
          );
  ...
  _numberOfDays(month, year) {
    var date = moment.utc([year, month, 0]);
    return date.toDate();
  }
  ...
  _getDisabledDays(day, month, year) {
    ...
    for (var i = 0; i <= monthDays; i++) {
      var date = moment.utc([year, month, i, 0]);
      if (date.day() == day) {
        disDays.push(date);
      }
    }
  ...
  _findAllEvents(events) {
    var today = moment.utc();
    var dd = (today.toDate() < 10 ? "0" : "") + today.toDate();
    var mm = (today.month() < 10 ? "0" : "") + (today.month() + 1);
    var isoToday = today.year() + "-" + mm + "-" + dd;
      ...
      if (result != undefined || result != null) {
        ...
        if (dayEvents.length <= 3) {
          ...
          result.style.boxShadow =
            "inset -4px 0 0 0 " + this.eventDayColor + "";
        }
        ...
        if (weight > 0) {
          ...
          for (var i in dayEvents) {
            ...
            boxShadow +=
              ...
              (parseInt(i) + 1) * weight +
              "px 0 0 0 " +
              (dayEvents[i].color == undefined
                ? dayEvents[i].category
                : dayEvents[i].color);
          }
        ...
        if (result.children.length == 0) {
          event +=
            '<div class="mp-cld-event"><div class="event">' +
            '<h3 class="' +
            item.category +
            '" ' +
          ...
          for (var i = 0; i < result.children.length; i++) {
            event +=
              '<span class="separator"></span><div class="event">' +
              '<h3 class="' +
              item.category +
              '" ' +
              (item.color ? 'style="color:' + item.color + '"' : "") +
